...
answers, assembling application drafts, and staging outbound messages) without asking per
action. Run these steps end-to-end in a single flow unless you hit a blocker.

You must pause for explicit confirmation before any **irreversible** action — chiefly
submitting a job application or sending a message/connection request — unless Juan has
explicitly pre-approved a specific batch in the current session.
...
1. **Always search "Forward Deployed Engineer" first.** Type it into the LinkedIn Jobs search
   ...
   cycling through the accepted title families. Continue autonomously across the full search
   set unless the browser session is blocked or Juan explicitly changes scope.
...
- **Landing-page inspection / no apply buttons visible:** When a LinkedIn job listing opens,
  read the landing-page details before taking any action. If, after the page loads, neither
  **Easy Apply** nor **Apply** is visible on the listing, treat the job as already applied to
  manually or agent-assisted, mark it as skipped/already handled, close the tab, and move on
  to the next LinkedIn Jobs listing.
...
1. Read the full inbound thread (or target profile) before drafting; identify the role,
   language, and any specific questions asked. Draft and stage the response autonomously; only
   stop when the next action would send it.
...
4. **Present the draft to Juan and pause for confirmation before sending** (sending is
   irreversible), unless Juan pre-approved the batch. Do not interrupt for intermediate
   drafting or formatting. After sending, confirm the message posted and record it.
...
1. Return to the main LinkedIn Jobs section.
2. Look for job listings that match my profile, location, and immigration preferences. Prefer Latin America.
3. Apply using both the "Easy Apply" and "Apply" options for external applications.
4. Apply to three job offerings that are autonomous and end-to-end.
5. Present a report when finished.

**Important**:

- You are authorized to fill out and submit any application form.
- If you access a third-party external job listings site or platform and Juan Jaramillo does not already have a profile there, autonomously create the account and populate the user profile with the verified information from Juan Jaramillo and this conversation.
- Prefer Google or GitHub authentication when those sign-in options are available.
- If manual account creation is required, use the email address <juanamillo@proton.me>.
- If verification codes or verification emails are sent, use the browser's already authenticated access to Juan Jaramillo's email services and accounts associated with LinkedIn to retrieve and confirm them.
