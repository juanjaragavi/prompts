...
> [!WARNING]
> While `GenerativeModel` _can_ support some OpenMaaS models, it is
...
> `get_gcp_access_token()` function above retrieves a _fresh_ token at the time
> it is called.
...
- **Cause**: OpenMaaS and Gemini models use **Dynamic Shared Quota (DSQ)**.
  ...
  that _your_ specific project quota is hit (though it can be).
