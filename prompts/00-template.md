 ...
 * Prioritize creativity and variety in creating drafts.
 ...
 * Ensure all content is original and does not copy text or screenshots from the provided examples, but rather uses them as inspiration for style and content strategy.
...
The ActiveCampaign variable to insert the first name of the subscriber in the email message, is ‘%FIRSTNAME%’. You should use it always where it applies. 

## Prompt

Generate the next `TOP - USA - TC - Generica` campaign. The user will input the ID.
